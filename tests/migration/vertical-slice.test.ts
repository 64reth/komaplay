...
test("Phase 1 uses React Router links and the canonical Tokon route", async () => {
  ...
  assert.equal(issueZeroFeatures.find((item) => item.id === "tokon")?.image, "/assets/clue-shield.png");
});
...
test("the migration production bundle has no Next or Vinext imports", async () => {
  const files = ["router-app/root.tsx", "router-app/components/Masthead.tsx", "router-app/components/FeatureStrip.tsx", "workers/app.ts"];
  for (const file of files) {
    const source = await readFile(file, "utf8");
    assert.doesNotMatch(source, /(?:from|import\()\s*["'](?:next|vinext)(?:\/|["'])/);
  }
});
