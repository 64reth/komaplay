...
test('production missing-column error is reproduced, then narrow migration repairs modular save and both submit paths', async () => {
  ...
  try {
    ...
    const run = async (intent: string, featureId = '', slug = 'timestamp-fixture', content = sections) => {
    ...
    assert.match(broken.data.error, /column "submitted_at" of relation "editorial_documents" does not exist/);
    ...
    await as(owner);
    const rejected=await run('submit',failingId,'rollback-fixture');assert.ok('error' in rejected.data);assert.match(rejected.data.error,/Fixture snapshot failure/);
    ...
    const duplicate=await run('save','','direct-submit-fixture');assert.ok('error' in duplicate.data);
