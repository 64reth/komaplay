...
function audit(page: Page) {
  ...
  page.on("requestfailed", (request) => {
    if (
      ...
      new URL(page.url() || "http://local").origin
    )
...
test("public vertical slice navigates through the rendered feature card", async ({
  page,
}) => {
  const errors = audit(page);
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "KOMA://PLAY" }),
  ).toBeVisible();
  ...
  const tokon = rail.locator('a[href="/features/tokon"]');
...
test("strip supports keyboard and horizontal scrolling", async ({ page }) => {
  ...
  const outline = await rail.evaluate(
  ...
  await rail.evaluate((element) =>
    element.scrollTo({ left: 120, behavior: "instant" }),
  );
  await expect
    .poll(() => rail.evaluate((element) => element.scrollLeft))
    .toBeGreaterThan(0);
