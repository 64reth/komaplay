...
import {draftDocument,parseComposerSections,serializeComposerSections,validateComposerSections,submissionBlocker} from './router-app/lib/editorial-alpha';
...
const sectionsOriginal=sections;
...
const router=createMemoryRouter([{id:'root',path:'/',loader:()=>({auth:{state:'signed-out',member:null},supabase:null}),Component:Outlet,children:[{path:'editorial',Component:Editorial,loader:()=>({state:'accepted',capabilities:{editorial:true,moderation:false},categories:[],drafts:[work],review:work.lifecycle_status==='submitted'?[work]:[]}),action:async({request})=>{
 ...
 if(issues.length)return {...submissionBlocker(intent==='submit-existing'?parseComposerSections(serializeComposerSections(sectionsOriginal)):sections),featureId:id,status:'draft'};
...
try {
  ...
  assert.match(await page.getByLabel('Submission requirements').innerText(), /1 submission requirement remaining/);
  ...
  assert.equal(await page.evaluate(() => document.activeElement?.id), 'body-alt-i');
  assert.equal(await page.getByRole('alert').innerText(), 'Draft saved, but not submitted. Add alt text to image section 5.');
  assert.equal(await page.getByLabel('Alt text (required)', {exact:true}).getAttribute('aria-invalid'), 'true');
  const altBox = await page.getByLabel('Alt text (required)', {exact:true}).boundingBox();
  assert.ok(altBox && altBox.y >= 0 && altBox.y < page.viewportSize().height);
