...
function SectionFields({ section, change, upload, issues }: { issues: DocumentIssue[]; section: ComposerSection; change: (patch: Partial<ComposerSection>) => void; upload: (event: React.ChangeEvent<HTMLInputElement>, id: string) => void }) {
  ...
  if (section.type === "image") return <>
    <label>Image URL or path<input {...attrs("url")} value={section.url ?? ""} onChange={e => change({ url: e.target.value })} maxLength={2000} /></label>
...
export function ArticleSectionBuilder({ sections, onChange, upload }: { sections: ComposerSection[]; onChange: (sections: ComposerSection[]) => void; upload: (event: React.ChangeEvent<HTMLInputElement>, id: string) => void }) {
  ...
  return <section aria-label="Article body" className="article-section-builder">
    ...
    {sections.map((section, index) => <fieldset className="composer-module" key={section.id} id={`section-${section.id}-section`} aria-invalid={issues.some(issue => issue.sectionId === section.id)} aria-describedby={issues.some(issue => issue.sectionId === section.id && issue.field === "section") ? `section-${section.id}-section-error` : undefined} tabIndex={-1}>
      ...
      <SectionFields issues={issues} section={section} change={patch => onChange(updateComposerSection(sections, section.id, patch))} upload={upload} />
