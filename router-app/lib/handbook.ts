...
export function safeReturnPath(
  ...
  try {
    ...
    if (
      !/^\/$|^\/(archive|search|about|documents(?:\/platform-notice)?|editorial|moderation|publishing|handbook|onboarding|profile(?:\/settings)?)$|^\/features\/[a-z0-9-]+(?:\/(?:workshop|correction))?$|^\/issues\/[a-z0-9-]+$/.test(
        url.pathname,
      )
    )
      return fallback;
