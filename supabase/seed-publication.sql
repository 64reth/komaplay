...
begin
 ...
 values('manga-margin','READ BETWEEN THE PANELS','published',issue,drops[2],2,(select id from public.categories where slug='manga'),(select id from public.content_formats where slug='discovery'),'open_panel','A reading notebook for the page between actions.','DEVELOPMENT EXAMPLE. Follow the spacing between panels and notice how it shapes the time you imagine passing. This placeholder is an in-house editorial prompt, not a report about a real release.','/assets/koma-vhs-v2.png','An in-house KOMA://PLAY archive tape used as an editorial placeholder','vhs') returning id into feature;
 ...
 values('demo-the-painted-frame','THE PAINTED FRAME','published',archive,(select id from public.weekly_drops where issue_id=archive limit 1),1,(select id from public.categories where slug='anime'),(select id from public.content_formats where slug='essay'),'archived','Development archive: the texture of painted backgrounds.','DEVELOPMENT ARCHIVE EXAMPLE. This finished panel is preserved with a closed Workshop. Later observations belong in a new feature; factual concerns use a private correction report.','/assets/koma-vhs-v2.png','A worn KOMA://PLAY VHS tape','vhs',now()-interval '30 days',now()-interval '29 days',now()-interval '35 days') returning id into feature;
