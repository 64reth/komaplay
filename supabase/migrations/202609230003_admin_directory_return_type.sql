-- Supabase auth.users.email is varchar in production. A plpgsql RETURN QUERY
-- requires its result types to match exactly, so normalise the private identity
-- field to the directory RPC's declared text type.
...
begin
 ...
 return query
 with directory as (
  select p.id,u.email::text as email,p.display_name,p.role,p.account_status,p.created_at,
