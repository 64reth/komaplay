-- Phase 4D issue-close selection repair.
-- Production features do not have created_at, so derive panel month from published/snapshot/updated/archive dates.
...
begin
 select i.id into v_issue
 from public.issues i
 where i.year=v_year and i.month=v_month
 order by case when i.status in ('current','finalising','published','open') then 0 when i.status='archived' then 2 else 1 end,
 ...
 select wd.id into v_drop
 from public.weekly_drops wd
 where wd.issue_id=v_issue and wd.status='published'
 order by wd.display_order desc, wd.week_number desc
 limit 1;
...
begin
 ...
 panel_date := coalesce(
   ...
   (select min(s.created_at) from public.editorial_document_snapshots s where s.feature_id=target and s.reason ilike '%published%'),
   feature.updated_at,
   feature.archived_at,
   now()
 );
...
begin
 ...
 if eligible_count = 0 then
   return jsonb_build_object('status','empty','message','No published panels are ready to close for this issue.','issueId',target_issue.id,'issueSlug',target_issue.slug,'issueTitle',target_title,'eligiblePanelCount',0,'publishedPanelCount',0,'alreadyArchivedPanelCount',0);
 end if;
 return jsonb_build_object('status','ready','issueId',target_issue.id,'issueSlug',target_issue.slug,'issueTitle',target_title,'year',target_year,'month',target_month,'eligiblePanelCount',eligible_count,'publishedPanelCount',published_count,'alreadyArchivedPanelCount',already_archived_count,'willCreateIssue',target_issue.id is null);
end $$;
